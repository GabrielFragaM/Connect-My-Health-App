...
class PdfApiHotel {
  static Future<File> generateTable(form) async {
    ...
    final headersTravel = ['Hotel', 'Detalhes'];
    ...
    if (data['possui_doencas_pontos'] == 0 &&
        data['possui_doencas_verificar'] ==
            2) {
      possui_doencas =
          data['possui_doencas'].toString().replaceAll('[', '').replaceAll(
              ']', '');
    }
    ...
    if (data['vacinas_pontos'] == 0) {
      vacinas = "${data['vacinas'].toString().replaceAll('[', '').replaceAll(
          ']', '')}, ${data['outras_vacinas']}";
    }
    ...
    if (data['ja_teve_doencas_pontos'] ==
        ...
        data['ja_teve_doencas_verificar'] ==
            2) {
      ja_teve_doencas =
          data['ja_teve_doencas'].toString().replaceAll('[', '').replaceAll(
              ']', '');
    }
    if (data['ja_teve_doencas_pontos'] ==
        3 &&
    ...
    if (data['pontos_sintomas'] != 0) {
      sintomas =
      "${data['sintomas'].toString().replaceAll('[', '').replaceAll(
          ']', '')}";
    }
    ...
    var users = [
      ...
      PdfScreen(Question: 'Faz uso de algum medicamento diário ?',
          Aswer: data['medicamento']),
      PdfScreen(Question: 'Tomou alguma vacina para esta viagem:',
          Aswer: tomou_vacinas_viagem_qual),
      PdfScreen(Question: 'Vacinas tomadas anteriormento:', Aswer: vacinas),
      PdfScreen(Question: 'Você sabe quais são as vacinas para o destino:',
          Aswer: voce_sabe_vacinas),
      PdfScreen(Question: 'Já teve doenças ?', Aswer: ja_teve_doencas),
      PdfScreen(
          Question: 'Ano que tive essas doenças:', Aswer: ano_teve_doencas),
      PdfScreen(Question: 'Sintomas nos últimos 7 dias:', Aswer: sintomas),
    ];
    final travel = [
      PdfScreen(QuestionTravel:  'Nome da Hospedagem:', AswerTravel: dataTravel['title']),
      PdfScreen(QuestionTravel: 'Cidade de Hospedagem:', AswerTravel: dataTravel['cidade']),
      PdfScreen(QuestionTravel:  'País de Hospedagem:', AswerTravel: dataTravel['pais']),
      PdfScreen(QuestionTravel: 'Tempo de duração (em dias):', AswerTravel: dataTravel['tempo']),
      PdfScreen(QuestionTravel:  'Data de Check-in:', AswerTravel: dataTravel['data']),
      PdfScreen(QuestionTravel: 'Motivo:', AswerTravel: dataTravel['motivo']),
      PdfScreen(QuestionTravel:  'Endereço da Hospedagem:', AswerTravel: dataTravel['endereco']),
    ];
